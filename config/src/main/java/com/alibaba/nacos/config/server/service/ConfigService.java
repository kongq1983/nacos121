...
public class ConfigService {
    ...
    static public boolean dump(String dataId, String group, String tenant, String content, long lastModifiedTs, String type) {
        ...
        try {
            final String md5 = Md5Utils.getMD5(content, Constants.ENCODE); // 数据库取出值的md5

            if (md5.equals(ConfigService.getContentMd5(groupKey))) { //md5未变
                dumpLog.warn(
            ...
            } else if (!STANDALONE_MODE || PropertyUtil.isStandaloneUseMysql()) { // 非nacos.standalone=true 或者 spring.datasource.platform=mysql
                DiskUtil.saveToDisk(dataId, group, tenant, content);  //把新值保存到磁盘
            }
            updateMd5(groupKey, md5, lastModifiedTs); //md5不存在或者新老md5不一样，则把m5更新到本地缓存
            return true;
