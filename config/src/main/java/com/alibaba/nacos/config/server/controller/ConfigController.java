...
public class ConfigController {
    ...
    public Boolean publishConfig(HttpServletRequest request, HttpServletResponse response,
        ...
        if (StringUtils.isBlank(betaIps)) {
            if (StringUtils.isBlank(tag)) { //默认从这里走
                persistService.insertOrUpdate(srcIp, srcUser, configInfo, time, configAdvanceInfo, false);
                EventDispatcher.fireEvent(new ConfigDataChangeEvent(false, dataId, group, tenant, time.getTime())); //通知listener
            } else {
    ...
    public ConfigAllInfo detailConfigInfo(HttpServletRequest request, HttpServletResponse response,
        ...
        // check params
        ParamUtils.checkParam(dataId, group, "datumId", "content");
        return persistService.findConfigAllInfo(dataId, group, tenant); //从config_info读取数据
    }
