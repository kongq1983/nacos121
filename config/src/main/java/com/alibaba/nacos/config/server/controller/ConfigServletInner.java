...
public class ConfigServletInner {
    ...
    public String doGetConfig(HttpServletRequest request, HttpServletResponse response, String dataId, String group,
        ...
        final String groupKey = GroupKey2.getKey(dataId, group, tenant); // $key+$group+$tenantid
        ...
        if (lockResult > 0) {
            ...
            try {
                ...
                if (cacheItem != null) {
                    ...
                    String configType = cacheItem.getType(); // 配置类型 text property yml等
                ...
                if (STANDALONE_MODE && !PropertyUtil.isStandaloneUseMysql()) {
                    ...
                    fis = new FileInputStream(file); //服务器端缓存文件
                    response.setDateHeader("Last-Modified", file.lastModified()); //文件最后修改时间
                }

                if (STANDALONE_MODE && !PropertyUtil.isStandaloneUseMysql()) { // spring.datasource.platform=mysql属性
                    ...
                    fis.getChannel().transferTo(0L, fis.getChannel().size(),
                        Channels.newChannel(response.getOutputStream())); //把数据放到response.getOutputStream()
                }
