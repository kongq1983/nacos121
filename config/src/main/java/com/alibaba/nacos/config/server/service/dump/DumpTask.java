    ...
    public boolean process(String taskType, AbstractTask task) {
        ...
        if (isBeta) {
            ...
            if (StringUtils.isBlank(tag)) {
                ConfigInfo cf = dumpService.persistService.findConfigInfo(dataId, group, tenant); // 从数据库取出  这个时候是修改后的值
                ...
                boolean result;
                if (null != cf) {
                    result = ConfigService.dump(dataId, group, tenant, cf.getContent(), lastModified, cf.getType()); //把新值保存到服务器本地文件
