    ...
    public void init() {
        ...
        Runnable clearConfigHistory = () -> {
            ...
            if (ServerListService.isFirstIp()) {
                try { // 默认getRetentionDays=30   可以通过设置环境变量nacos.config.retention.days改变
                    Timestamp startTime = getBeforeStamp(TimeUtils.getCurrentTime(), 24 * getRetentionDays()); //开始时间是 当前时间-24*30小时 也就是30天之前数据
                    int totalCount = persistService.findConfigHistoryCountByTime(startTime); // 30天之前数据总数
                    if (totalCount > 0) {
                        int pageSize = 1000;
