...
public class NamespaceController {
    ...
    /** persistService.configInfoCount 获取某个namespace的配置项数量
     * Get namespace list
     * 获取namespace列表 从tenant_info表获取
    ...
    public RestResult<List<Namespace>> getNamespaces(HttpServletRequest request, HttpServletResponse response) {
        ...
        namespaces.add(namespace0); // 添加默认的public namespace
        for (TenantInfo tenantInfo : tenantInfos) {
            int configCount = persistService.configInfoCount(tenantInfo.getTenantId()); // 获取该namespace的配置项数量
    ...
    public Boolean createNamespace(HttpServletRequest request, HttpServletResponse response,
        ...
        // TODO 获取用kp
        if(StringUtils.isBlank(namespaceId)){
            namespaceId = UUID.randomUUID().toString(); // 命名空间ID不传，默认生成
        } else {
            namespaceId = namespaceId.trim();
            if (!namespaceIdCheckPattern.matcher(namespaceId).matches()) { // id规则要匹配 ^[\w-]+
                return false;
            }
            if (namespaceId.length() > NAMESPACE_ID_MAX_LENGTH) { //id长度不能大于128位
                return false;
            }
            if(persistService.tenantInfoCountByTenantId(namespaceId) > 0){ //判断namespaceId是否存在
                return false;
            }
        }
